...
class ServiceModel extends Model
{
	public function __construct()
	{
		...
		list( $this->_R['id'],$this->_R['name'],$this->_R['first']) = $this->getRequest()->getParam( array( isset( $_GET['id'] ) ? $_GET['id'] : null,isset( $_POST['name'] ) ? $_POST['name'] : null,isset( $_POST['first'] ) ? $_POST['first'] : null ) );
	}

	public function findAll()
	{
		return parent::select(array('id','name','content','first'),array('limit'=>$this->_limit,'order'=>'date DESC'));
	}

	public function findAddGoodsService()
	{
		$_service = parent::select(array('id','name'));
		$_serviceArr = '';
		foreach($_service as $_key=>$_value)
		{
			$_serviceArr[$_value->id] = $_value->name;
		}
		return $_serviceArr;
	}
	...
	{
		$_service = parent::select(array('id','first'));
		foreach($_service as $_key=>$_value)
		{
			if($_value->first == 1)
			{
