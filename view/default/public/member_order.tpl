...
<div id="main">
	...
	<table id="cart" cellspacing="1">
		...
		<tr><td><a href="?a=member&m=order_details&id={$value->id}">{$value->ordernum}</a></td><td>{$value->date}</td><td>{$value->price}/元</td><td>{$value->order_state}，{$value->order_pay}，{$value->order_delivery}</td><td>取消</td></tr>
		{/foreach}
	</table>
