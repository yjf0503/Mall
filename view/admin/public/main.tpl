...
<div id="list">
	<table>
		...
		<tr><td>待发货订单：</td><td>{$UndeliveredGoodsCount}</td><td>未确认订单：</td><td>{$UnconfirmedGoodsCount}</td></tr>
		<tr><td>未支付订单：</td><td>{$UnpayedGoodsCount}</td><td>申请退款订单：</td><td>{$RefundGoodsCount}</td></tr>
	</table>
	...
	<table>
		...
		<tr><td>服务器操作系统：</td><td>{php}echo PHP_OS{/php}</td><td>服务器软件：</td><td>{php}echo $_SERVER["SERVER_SOFTWARE"]{/php}</td></tr>
	</table>
</div>
